...
const MYCODE_CONCENTRATES = {
  revitalising: { code: '01', name: 'Revitalising Facial Concentrate', purpose: 'Tired, dull, stressed skin' },
  comforting: { code: '02', name: 'Comforting Facial Concentrate', purpose: 'Sensitive, reactive, irritated skin' },
  hydronourishing: { code: '03', name: 'Hydro-Nourishing Facial Concentrate', purpose: 'Dry, dehydrated, comfort-lacking skin' },
  depigmenting: { code: '04', name: 'Depigmenting Facial Concentrate', purpose: 'Pigmentation, dark spots, uneven tone' },
  replenish: { code: '05', name: 'Replenish Facial Concentrate', purpose: 'Wrinkles, loss of volume, density support' },
  reaffirming: { code: '06', name: 'Reaffirming Facial Concentrate', purpose: 'Lifting, firmness, facial contours' },
  retinol: { code: '07', name: 'Renewing Retinol Facial Concentrate', purpose: 'Photoaging, texture, renewal, wrinkles' },
  neck: { code: '08', name: 'Neck & Décolleté Remodelling Concentrate', purpose: 'Neck, décolleté, contour remodelling' },
}
...
function step(id, name, line, category, instruction = '') {
  return { id, name, line, category, instruction }
}
...
function buildPreparation(lang) {
  ...
  return [
    step('ecc_micellar', 'ECC Remover Micellar Eyes & Face', 'ESSENTIAL CARE CONCEPT', isRU ? 'Очищение' : isEN ? 'Cleansing' : 'Reinigung', isRU ? 'Деликатное очищение глаз, лица, шеи и декольте.' : isEN ? 'Gentle cleansing of eyes, face, neck and décolleté.' : 'Sanfte Reinigung von Augen, Gesicht, Hals und Dekolleté.'),
    step('ecc_3d_texture_cleanser', 'ECC Remover 3D Texture Cleanser', 'ESSENTIAL CARE CONCEPT', isRU ? 'Глубокое очищение' : isEN ? 'Deep cleansing' : 'Tiefenreinigung', isRU ? 'Проработать кожу для глубокого очищения и подготовки к активной фазе.' : isEN ? 'Work into the skin for deep cleansing and preparation before the active phase.' : 'Für Tiefenreinigung und Vorbereitung der Aktivphase einarbeiten.'),
    step('ecc_mist', 'ECC Remover Mist', 'ESSENTIAL CARE CONCEPT', isRU ? 'Тонизация' : isEN ? 'Toning' : 'Tonisierung', isRU ? 'Восстановить комфорт кожи и подготовить её к пилингу.' : isEN ? 'Restore skin comfort and prepare for peeling.' : 'Hautkomfort wiederherstellen und auf das Peeling vorbereiten.'),
    step('ecc_multi_acid_peel', 'ECC Renewal Multi-Acid Peel', 'ESSENTIAL CARE CONCEPT', isRU ? 'Пилинг' : isEN ? 'Peeling' : 'Peeling', isRU ? 'Экспозицию адаптировать к состоянию и чувствительности кожи.' : isEN ? 'Adjust exposure time to skin condition and sensitivity.' : 'Einwirkzeit an Hautzustand und Empfindlichkeit anpassen.'),
  ]
}
...
function buildFinalCare(lang) {
  ...
  return [
    step('ecc_repair_shield_spf50', 'ECC Repair Shield Final Cream SPF50', 'ESSENTIAL CARE CONCEPT', isRU ? 'Финальная защита' : isEN ? 'Final protection' : 'Abschlussschutz', isRU ? 'Завершить процедуру защитным SPF50.' : isEN ? 'Finish the treatment with SPF50 protection.' : 'Behandlung mit SPF50-Schutz abschließen.'),
    step('ecc_lip_balm', 'ECC Repair Lip Balm', 'ESSENTIAL CARE CONCEPT', isRU ? 'Губы' : isEN ? 'Lips' : 'Lippen', isRU ? 'Защитить и восстановить губы.' : isEN ? 'Protect and restore the lips.' : 'Lippen schützen und regenerieren.'),
  ]
}
...
function buildSimpleProtocol(input, decision, variant) {
  ...
  return {
    ...
    equipment: variant.device ? [{ name: variant.device, usage: '' }] : [],
