...
export function buildHomecareRecommendation(formData = {}) {
  ...
  const isLowSensitivity = has(text, [
    'низкая чувств',
    'низкой чувств',
    'low sensitivity',
  ...
  if (primary === 'BECLARITY') {
    ...
    weeklySupport = clean([
      isDry ? product('hydro_repairer_serum', lang) : null,
    ])
    ...
    avoid = [
      langText(lang, 'Не выходить на солнце без SPF', 'Sonne ohne SPF vermeiden', 'Avoid sun exposure without SPF'),
      langText(lang, 'Не использовать агрессивные пилинги без рекомендации косметолога', 'Aggressive Peelings ohne Empfehlung der Kosmetikerin vermeiden', 'Avoid aggressive peels without a cosmetologist recommendation'),
    ]
  }
  ...
  return {
    ...
    primaryLine: primary,
    secondaryLines: usedLines.filter(
      (line) => line !== primary && line !== 'MYCODE ADVANCED' && line !== 'SUMMESUN'
    ),
    lines: usedLines,
    priorities,
    morning,
