...
export function buildHomecareRecommendation(data) {
  ...
  // Priority logic: main aesthetic concern first
  ...
  if ((hasWrinkles || age >= 45) && primaryLine !== 'CELL') {
    supportLines.push('CELL')
  }

  if (hasPigmentation && primaryLine !== 'BECLARITY') {
    supportLines.push('BECLARITY')
  }
  ...
  if (primaryLine === 'CELL') {
    ...
    if (isDry) {
      morning.unshift(product('hydraluronic_serum_gel'))
      evening.unshift(product('hydraluronic_serum_gel'))
    }

    if (isSensitive) {
      morning.unshift(product('nicely_gentle_cleanser'))
      evening.unshift(product('nicely_gentle_cleanser'))
    }
  ...
  if (primaryLine === 'BECLARITY') {
    ...
    priorities = [
      ...
      langText(lang, 'Строгая ежедневная SPF-защита', 'Konsequenter täglicher UV-Schutz', 'Strict daily SPF protection'),
    ]
