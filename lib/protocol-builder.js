...
import { PROTOCOL_LIBRARY } from "./protocol-library.js";
...
function resolveMycodeVariant({ mainLine, mainGoal, ageSubGoal, additionalGoal }) {
...
function findProtocolByLine(line, variant = null) {
  if (!PROTOCOL_LIBRARY) return null;

  const protocols = Array.isArray(PROTOCOL_LIBRARY)
    ? PROTOCOL_LIBRARY
    : Object.values(PROTOCOL_LIBRARY);

  return (
    protocols.find((protocol) => {
      const protocolLine = protocol.line || protocol.mainLine || protocol.category;
      const protocolVariant = protocol.variant || protocol.mycodeVariant || null;

      if (variant) {
        return protocolLine === line && String(protocolVariant) === String(variant);
      }

      return protocolLine === line;
    }) || null
  );
}
...
function resolveHomecare({ mainLine, supportingLines }) {
  const lines = [mainLine, ...supportingLines];
  ...
  return lines.map((line) => ({
    ...
    role:
      line === mainLine
        ? "Основная домашняя линия после процедуры."
        : "Поддерживающая домашняя линия по дополнительной задаче.",
  }));
}
...
export function buildProtocol(input = {}) {
  ...
  const officialProtocol =
    findProtocolByLine(mainLine, mycodeVariant) ||
    findProtocolByLine(mainLine) ||
    buildDefaultProtocol({ mainLine, mycodeVariant });
  ...
  return {
    ...
    strategy: {
      ...
      mycodeVariant,
      supportingLines,
      supportingLineLabels: supportingLines.map((line) => LINE_LABELS[line] || line),
      ageModifier,
    },
